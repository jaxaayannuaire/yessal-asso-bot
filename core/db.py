...
class DatabaseManager:
        ...
        try:
            ...
            conn.execute("""
                CREATE TABLE IF NOT EXISTS cache_contacts (
                    id VARCHAR PRIMARY KEY, firstname VARCHAR, lastname VARCHAR,
                    phone VARCHAR, email VARCHAR, status VARCHAR, last_sync TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS cache_members (
                    id VARCHAR PRIMARY KEY, morphy VARCHAR, lastname VARCHAR, firstname VARCHAR,
                    email VARCHAR, phone VARCHAR, status VARCHAR, date_fin VARCHAR,
                    last_sync TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS cache_memberships (
                    id VARCHAR PRIMARY KEY, member_id VARCHAR, date_subscription VARCHAR,
                    date_start VARCHAR, date_end VARCHAR, amount DECIMAL, status VARCHAR,
                    last_sync TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS cache_contributions (
                    id VARCHAR PRIMARY KEY, ref VARCHAR, member_id VARCHAR, amount DECIMAL,
    ...
    def sync_contacts(self, contacts_data):
        ...
        try:
            ...
            for c in contacts_data:
                ...
                conn.execute("INSERT INTO cache_contacts (id, firstname, lastname, phone, email, status) VALUES (?, ?, ?, ?, ?, ?)", [c_id, firstname, lastname, phone, email, status])
    ...
    def search_contacts(self, query):
        ...
        return conn.execute("SELECT id, firstname, lastname, phone FROM cache_contacts WHERE LOWER(firstname) LIKE ? OR LOWER(lastname) LIKE ? OR phone LIKE ? LIMIT 10", [search_term, search_term, search_term]).fetchall()
    ...
    def sync_members(self, members_data):
        ...
        try:
            ...
            for m in members_data:
                ...
                conn.execute("INSERT INTO cache_members (id, morphy, lastname, firstname, email, phone, status, date_fin) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [m_id, morphy, lastname, firstname, email, phone, status, date_fin])
    ...
    def search_members(self, query):
        conn = self.connect()
        search_term = f"%{query.lower()}%"
        return conn.execute("SELECT id, firstname, lastname, phone, status, date_fin FROM cache_members WHERE LOWER(firstname) LIKE ? OR LOWER(lastname) LIKE ? OR phone LIKE ? LIMIT 10", [search_term, search_term, search_term]).fetchall()
    ...
    def sync_memberships(self, memberships_data):
        ...
        try:
            ...
            for sub in memberships_data:
                ...
                conn.execute("INSERT INTO cache_memberships (id, member_id, date_subscription, date_start, date_end, amount, status) VALUES (?, ?, ?, ?, ?, ?, ?)", [sub_id, member_id, date_sub, date_start, date_end, amount, status])
    ...
    def sync_contributions(self, contributions_data):
        ...
        try:
            ...
            for contrib in contributions_data:
                ...
                conn.execute("INSERT INTO cache_contributions (id, ref, member_id, amount, date_payment, type) VALUES (?, ?, ?, ?, ?, ?)", [c_id, ref, member_id, amount, date_payment, c_type])
