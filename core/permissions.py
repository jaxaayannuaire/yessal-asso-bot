...
ROLE_PERMISSIONS = {
    ...
    ROLE_PRESIDENT: {
        ...
        "members.create", "contacts.create", "thirdparties.create",
    },
    ROLE_TRESORIER: {
...
def ensure_schema(db) -> None:
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS bot_users (
        telegram_id VARCHAR PRIMARY KEY, username VARCHAR,
        role VARCHAR DEFAULT 'user', dolibarr_contact_id VARCHAR,
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS dolibarr_users (
        id VARCHAR PRIMARY KEY, login VARCHAR, firstname VARCHAR, lastname VARCHAR,
        email VARCHAR, active BOOLEAN DEFAULT true, last_sync TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
    conn.execute("""CREATE TABLE IF NOT EXISTS dolibarr_groups (
        id VARCHAR PRIMARY KEY, name VARCHAR, ref VARCHAR,
        group_type VARCHAR DEFAULT 'unknown', last_sync TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS dolibarr_user_groups (
        dolibarr_user_id VARCHAR NOT NULL, dolibarr_group_id VARCHAR NOT NULL,
        role VARCHAR, group_type VARCHAR DEFAULT 'unknown',
...
def sync_from_dolibarr(client, db) -> tuple[bool, str]:
    ...
    for group in groups or []:
        ...
        group_type = "technical" if normalized in TECHNICAL_GROUP_NAMES else ("business" if role else "other")
        group_meta[gid] = (role, group_type)
        conn.execute("""INSERT INTO dolibarr_groups
            (id, name, ref, group_type, last_sync) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)""",
    ...
    for user in users or []:
        ...
        conn.execute("""INSERT INTO dolibarr_users
            (id, login, firstname, lastname, email, active, last_sync)
        ...
        for membership in memberships or []:
            gid = str(_first(membership, "id", "rowid"))
            if not gid or gid == "None":
                continue
            role, group_type = group_meta.get(gid, (
                role_from_group_name(_first(membership, "name", "nom", "label", default="")), "unknown"))
            conn.execute("""INSERT OR REPLACE INTO dolibarr_user_groups
                (dolibarr_user_id, dolibarr_group_id, role, group_type, last_sync)
                VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)""",
                [uid, gid, role, group_type])
            linked_memberships += 1
    return True, f"✅ Synchronisation terminée : {synced_users} utilisateurs, {len(group_meta)} groupes, {linked_memberships} appartenances."

def get_roles_for_dolibarr_user(db, dolibarr_user_id: str) -> set[str]:
    ensure_schema(db)
    rows = db.connect().execute("""SELECT role FROM dolibarr_user_groups
        WHERE dolibarr_user_id = ? AND group_type = 'business' AND role IS NOT NULL""",
        [str(dolibarr_user_id)]).fetchall()
...
def has_permission(roles: Iterable[str], permission: str) -> bool:
    return any("*" in ROLE_PERMISSIONS.get(role, set()) or permission in ROLE_PERMISSIONS.get(role, set()) for role in roles)
...
def find_dolibarr_user(db, target: str):
    ...
    if not is_valid_dolibarr_user_id(value):
        return None
    return db.connect().execute("""SELECT id, login, firstname, lastname, email, active
        FROM dolibarr_users WHERE id = ? LIMIT 1""", [value]).fetchone()
...
def find_group_by_role(db, role: str):
    ...
    if not name:
        return None
    return db.connect().execute("""SELECT id, name, ref, group_type
        FROM dolibarr_groups WHERE group_type = 'business'
        AND (upper(trim(name)) = upper(?) OR upper(trim(ref)) = upper(?)) LIMIT 1""",
        [name, name]).fetchone()
