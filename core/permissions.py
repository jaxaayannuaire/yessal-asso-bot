...
def ensure_schema(db) -> None:
    ...
    if "group_type" not in cols:
        conn.execute(
            "ALTER TABLE dolibarr_groups ADD COLUMN group_type VARCHAR DEFAULT 'unknown'"
        )
    ...
    if "group_type" not in cols:
        conn.execute(
            "ALTER TABLE dolibarr_user_groups ADD COLUMN group_type VARCHAR DEFAULT 'unknown'"
        )
...
def sync_from_dolibarr(client, db) -> tuple[bool, str]:
    ...
    # Only the local mirror is replaced. Dolibarr remains the source of truth.
    ...
    for group in groups or []:
        ...
        group_type = (
            "technical"
            if normalized in TECHNICAL_GROUP_NAMES
            else ("business" if role else "other")
        )
        group_meta[gid] = (role, group_type)

        # Explicit columns: safe with existing DuckDB migrations where
        # group_type may have been appended after last_sync.
        conn.execute(
            """
            INSERT INTO dolibarr_groups
                (id, name, ref, group_type, last_sync)
    ...
    for user in users or []:
        ...
        active = (
            bool(int(active_value))
            if str(active_value).isdigit()
            else bool(active_value)
        )

        conn.execute(
            """
            INSERT INTO dolibarr_users
        ...
        if not ok_memberships:
            logger.warning(
                "Impossible de lire les groupes de l'utilisateur %s: %s",
                uid,
                memberships,
            )
            continue
        ...
        for membership in memberships or []:
            ...
            role, group_type = group_meta.get(
                gid,
                (
                    role_from_group_name(
                        _first(membership, "name", "nom", "label", default="")
                    ),
                    "unknown",
                ),
            )
            ...
            conn.execute(
                """
                INSERT OR REPLACE INTO dolibarr_user_groups
...
def has_permission(roles: Iterable[str], permission: str) -> bool:
    return any(
        "*" in ROLE_PERMISSIONS.get(role, set())
        or permission in ROLE_PERMISSIONS.get(role, set())
        for role in roles
    )
...
def find_group_by_role(db, role: str):
    ...
    return db.connect().execute(
        """
        SELECT id, name, ref, group_type
        FROM dolibarr_groups
        WHERE group_type = 'business'
          AND (
              upper(trim(name)) = upper(?)
              OR upper(trim(ref)) = upper(?)
          )
        LIMIT 1
        """,
        [name, name],
