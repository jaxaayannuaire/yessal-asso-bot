...
async def sync_contacts_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Commande /sync : Force la mise à jour du cache DuckDB depuis Dolibarr"""
    user_id = str(update.effective_user.id)
    
    auth = AuthManager()
    user = auth.get_user(user_id)
    if not user or user[2] not in ['super_admin', 'president', 'tresorier']:
        await update.message.reply_text("⛔ Vous n'avez pas l'autorisation de synchroniser la base.")
        return
...
async def search_contact_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Commande /contact <nom/numero> : Cherche un contact dans DuckDB"""
    user_id = str(update.effective_user.id)
