...
def _display_name(contact):
    ...
    return f"{firstname} {lastname}".strip() or contact.get("name") or contact.get("societe") or "Sans nom"


def _format_field(key, value):
    label = FIELD_LABELS.get(key, key.replace("options_", "").replace("_", " ").strip().capitalize())
...
def build_contact_results_text(results):
    if not results:
        return "📇 *RÉSULTATS CONTACTS*\n\nAucun contact ne correspond aux critères."

    lines = ["📇 *RÉSULTATS CONTACTS*", f"👥 {len(results)} résultat(s)"]
...
def _iter_detail_fields(contact):
    seen_labels = set()
    ordered = (
        "type", "type_contact", "poste", "societe", "company", "ref",
    ...
    for key, value in contact.items():
        if key in METADATA_FIELDS or key in ordered or not _has_value(value):
            continue
    ...
    if isinstance(options, dict):
        for key, value in options.items():
            ...
            label = FIELD_LABELS.get(clean_key, clean_key.replace("_", " ").capitalize())
            if label in seen_labels:
                continue
            seen_labels.add(label)
...
def build_contact_detail_text(contact):
    contact_id = contact.get("id")
    suffix = f" (ID : {contact_id})" if _has_value(contact_id) else ""
    lines = [f"📇 *DÉTAIL CONTACT*", "", f"*{_display_name(contact)}*{suffix}"]
